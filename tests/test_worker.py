...
def test_worker_runs_all_stages_from_start(tmp_path):
    ...
    with patch("pipeline.worker._run_stage", side_effect=lambda stage, ctx, cfg, prov: (
...
def test_worker_failure_on_download_error(tmp_path):
    from pipeline.worker import _process_job
    minio = MagicMock()
    minio.download_processing_file.side_effect = Exception("network error")
    mock_run = MagicMock()
    http_calls = []

    with patch("pipeline.worker._run_stage", mock_run), \
         patch("pipeline.worker._post_callback", side_effect=lambda url, body: http_calls.append(body)), \
         patch("pipeline.worker.runner._build_providers_for_stage", return_value={}):
        msg = make_mq_message()
        _process_job(msg, minio=minio, workspace=tmp_path, dagservice_url="http://localhost:8080")
    ...
    assert http_calls[0]["stage"] == "download"
    assert http_calls[0]["status"] == "failed"
    assert "network error" in http_calls[0]["error_msg"]
    mock_run.assert_not_called()


def test_worker_failure_on_invalid_resume_stage(tmp_path):
    from pipeline.worker import _process_job
    minio = MagicMock()
    minio.download_processing_file = MagicMock()
    mock_run = MagicMock()
    ...
    with patch("pipeline.worker._run_stage", mock_run), \
         patch("pipeline.worker._post_callback", side_effect=lambda url, body: http_calls.append(body)), \
         patch("pipeline.worker.runner._build_providers_for_stage", return_value={}):
        msg = make_mq_message(resume_from="nonexistent")
        _process_job(msg, minio=minio, workspace=tmp_path, dagservice_url="http://localhost:8080")

    assert len(http_calls) == 1
    assert http_calls[0]["stage"] == "routing"
    assert http_calls[0]["status"] == "failed"
    assert "nonexistent" in http_calls[0]["error_msg"]
    mock_run.assert_not_called()


def test_worker_failure_on_upload_error(tmp_path):
    from pipeline.worker import _process_job
    minio = MagicMock()
    minio.download_processing_file = MagicMock()
    minio.upload_job_outputs.side_effect = Exception("s3 error")
    http_calls = []

    with patch("pipeline.worker._run_stage", side_effect=lambda stage, ctx, cfg, prov: ctx), \
         patch("pipeline.worker._post_callback", side_effect=lambda url, body: http_calls.append(body)), \
         patch("pipeline.worker.runner._build_providers_for_stage", return_value={}):
        msg = make_mq_message()
        _process_job(msg, minio=minio, workspace=tmp_path, dagservice_url="http://localhost:8080")

    upload_failures = [c for c in http_calls if c.get("stage") == "upload"]
    assert len(upload_failures) == 1
    assert upload_failures[0]["status"] == "failed"
    assert "s3 error" in upload_failures[0]["error_msg"]
